...
The user-facing entry points are (i) a CLI command \texttt{python -m researcher ask "\ldots"} with optional \texttt{-{}-sources} and \texttt{-{}-no-cache} flags, (ii) a FastAPI HTTP endpoint \texttt{POST /research}, and (iii) a Streamlit UI that calls the API. The HTTP endpoint accepts a JSON body with \texttt{question}, \texttt{sources}, and \texttt{no\_cache} fields. All entry points return a synthesised answer plus numbered citations.
...
The provider stack is configured through environment variables rather than hard-coded in the application: the repository supports Anthropic, OpenAI, and Gemini LLMs, with Tavily, Serper, or DuckDuckGo for web search. The current demonstration configuration uses OpenAI and Tavily, while the offline demo uses deterministic fakes. Our code calls exactly four functions from the provided \texttt{ai/} package: \texttt{ai.fetch\_wikipedia}, \texttt{ai.fetch\_arxiv}, \texttt{ai.fetch\_web}, and \texttt{ai.synthesize}. The public interface of the \texttt{ai/} package is completely unchanged; we wrap each call in our own service layer without modifying its contract.
...
The service and integration layer was collaborative. Gunel implemented the initial API and AI integration, completed the Streamlit UI and provider-selection workflow, and contributed the bilingual UI/API iteration in PR~\#15. The final merged version retained the English UI and provider selection after the temporary Azerbaijani option was removed. Nazrin then connected that work to the orchestrator, cache, CLI, validation, retries, timeouts, tests, and Docker packaging. The final behavior therefore reflects iterative integration rather than one member owning every line of the engine or AI service.
...
Figure 1 shows the layered module structure. The \texttt{ai/} boundary (dashed) marks the provided package; everything inside our \texttt{src/} is original SE work.
...
\begin{itemize}[noitemsep]
  ...
  \item \texttt{src/services/cache.py} --- \texttt{CacheService} normalises the \texttt{(source, query)} key to a SHA-256 hex digest before delegating to the backend.
  ...
  \item \texttt{app.py} --- Streamlit frontend with provider/source selection; the selected provider is forwarded to the API for that request.
\end{itemize}
...
\textit{Machine: Windows, Python 3.11, network live, Wikipedia returned HTTP 403 (so four source results were counted). Cache cleared before each run. Command: \texttt{python scripts/bench.py}.}
...
We use a \textbf{filesystem JSON} cache rather than SQLite. Each cache entry is a single \texttt{.json} file under \texttt{.cache/}, named by the SHA-256 hex digest of \texttt{"\{source\}::\{canonicalized\_query\}"}. The JSON structure mirrors the \texttt{CacheEntry} Pydantic model:

\begin{lstlisting}[style=python,numbers=none,basicstyle=\ttfamily\small]
{
  "source": "wikipedia",
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Key:} \texttt{SHA256(f"\{source\}::\{query.strip().lower()\}")} --- case-insensitive, whitespace-normalised.
...
All 51 tests are fully offline. HTTP and AI calls are replaced with local fakes and test clients; the provided \texttt{tests/test\_ai\_smoke.py} passes unchanged.
...
\begin{itemize}
  ...
  \item \textbf{No LLM failover:} if the selected LLM provider fails, the answer is \texttt{None}. The UI lets the user choose a configured provider per request, but it does not silently switch providers.
...
\begin{itemize}
  \item \textbf{\texttt{tests/test\_api.py}} (Nazrin, PR \#13): An AI assistant drafted the initial test file structure. The author reviewed all test cases, confirmed offline-only behaviour (no live HTTP), verified all 51 tests pass locally, and added edge-case assertions for invalid source names.
  ...
  \item \textbf{PR \#15} (Gunel): Added the bilingual Azerbaijani/English UI/API iteration, Streamlit provider configuration, and English UI cleanup. The final merged version retained the English UI and LLM-provider selection; the temporary Azerbaijani option was removed before the final state.
  \item \textbf{Documentation and formatting} (Orkhan): AI assistance was used for limited LaTeX formatting, correcting title-page \texttt{tabularx} layout overflow, and extracting exact GitHub LOC metrics. All project data, limitations, and architectural details were written and verified by the team.
\end{itemize}

We affirm that every team member can defend every line of code in this repository during the oral examination.

% =====================================================================
\section{Team Workload \& Contribution}\label{sec:contribution}

The GitHub Insights record reports the following final statistics:

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
\textbf{Member} & \textbf{Commits} & \textbf{Added} & \textbf{Deleted} & \textbf{Primary ownership} \\
\midrule
Nazrin Burziyeva & 13 & 1,278 & 542 & Service integration, CLI, tests, Docker \\
Gunel Garayzade & 6 & 1,395 & 373 & Initial AI/API integration, Streamlit UI \\
Orkhan Nuriyev & 5 & 2,642 & 16 & Orchestration, storage, config, benchmark \\
\bottomrule
\end{tabular}
\end{center}

Gunel implemented the initial AI/API integration, completed the Streamlit UI
and provider-selection workflow, and contributed PR~\#15. Nazrin completed the surrounding engine and service integration,
validation, retries, timeouts, CLI, tests, and Docker packaging. Orkhan owned
the infrastructure foundation: typed settings, Pydantic models, concurrent
orchestration, cache backends, storage, and the benchmark. The engine and AI
integration were collaborative, and the contribution statement records the
corresponding pull requests and review history.
