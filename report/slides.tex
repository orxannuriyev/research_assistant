...
\begin{frame}{Architecture}
  ...
  \begin{tikzpicture}[
    ...
    % AI module
    \node[aibox,below=of svc] (ai) {\textbf{provided \texttt{ai/}}\\(Anthropic + Tavily)};
...
\begin{frame}{Three design decisions we can defend}
  ...
  \begin{block}{Expect to be asked}
    ``Why did you not use a database for cache?'' Because TTL-based file eviction is sufficient for ephemeral research queries.
  \end{block}
\end{frame}

% ========== SLIDE 5: CONCURRENCY (THE BENCHMARK) =====================
\begin{frame}{Concurrency: the benchmark}
  \textbf{Workload:} 5 research questions, 4 active sources (Wikipedia returned 403).
  ...
  \vspace{0.3em}
  {\footnotesize\textit{Numbers run on macOS, Python 3.11.16 via \texttt{python scripts/bench.py}.}}
\end{frame}
...
\begin{frame}{Testing}
  ...
  \begin{itemize}\setlength\itemsep{0.3em}
    ...
    \item Test count: 40 own + 10 smoke (50 passed)
  ...
  \vspace{0.6em}
  \begin{block}{Tooling}
    \texttt{pytest} + \texttt{pytest-cov} + \texttt{pytest-mock} + \texttt{unittest.mock}.
  \end{block}
\end{frame}
...
\begin{frame}{Limitations \& what we'd do next}
  ...
  \begin{itemize}\setlength\itemsep{0.3em}
    \item No multi-provider failover; if Anthropic is down, we return no answer.
  ...
  \begin{enumerate}\setlength\itemsep{0.3em}
    \item Add an Anthropic $\to$ OpenAI fallback inside \texttt{AIService}.
...
\begin{frame}{Team contributions}
  ...
  \begin{tabularx}{\linewidth}{lXl}
    ...
    \midrule
    Orkhan Nuriyev   & Core Orchestration, Storage, Config   & 2642 LOC, 3 PRs, 4 Commits \\
    Nazrin Burziyeva & Robustness, CLI, Testing Framework    & 1169 LOC, 6 PRs, 12 Commits \\
    Gunel Garayzade  & AI Service, API Integration, Swagger  & 944 LOC, 3 PRs, 4 Commits \\
    \bottomrule
  \end{tabularx}
  \end{center}
